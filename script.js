class PongGame {
    ...
    update() {
        ...
        if (this.keys['x'] && this.leftPaddle.x > 0) {
            this.leftPaddle.x -= this.leftPaddle.speed;
        }
        if (this.keys['c'] && this.leftPaddle.x < this.canvas.width / 2 - this.leftPaddle.width) {
            this.leftPaddle.x += this.leftPaddle.speed;
        }
        ...
        if (this.keys['arrowleft'] && this.rightPaddle.x > this.canvas.width / 2) {
            this.rightPaddle.x -= this.rightPaddle.speed;
        }
        if (this.keys['arrowright'] && this.rightPaddle.x < this.canvas.width - this.rightPaddle.width) {
            this.rightPaddle.x += this.rightPaddle.speed;
        }
        ...
        if (this.checkPaddleCollision(this.leftPaddle) || this.checkPaddleCollision(this.rightPaddle)) {
            ...
            // Apply boost effect - increase speed slightly
            this.ball.dx *= this.ball.boostMultiplier;
            this.ball.dy *= this.ball.boostMultiplier;
            
            // Limit maximum speed
            const currentSpeed = Math.sqrt(this.ball.dx * this.ball.dx + this.ball.dy * this.ball.dy);
            if (currentSpeed > this.ball.maxSpeed) {
                const ratio = this.ball.maxSpeed / currentSpeed;
                this.ball.dx *= ratio;
                this.ball.dy *= ratio;
            }
    ...
    checkPaddleCollision(paddle) {
        ...
        // Determine rotation angle based on paddle
        const isLeftPaddle = paddle === this.leftPaddle;
        const angle = isLeftPaddle ? -Math.PI / 4 : Math.PI / 4;
        
        // Transform ball position to paddle's local coordinate system
        const dx = this.ball.x - paddleCenterX;
        const dy = this.ball.y - paddleCenterY;
        
        // Rotate coordinates back to axis-aligned
        const cos = Math.cos(-angle);
        const sin = Math.sin(-angle);
        const localX = dx * cos - dy * sin;
        const localY = dx * sin + dy * cos;
        
        // Check collision in local coordinate system
        const halfWidth = paddle.width / 2;
        const halfHeight = paddle.height / 2;
        
        return Math.abs(localX) <= halfWidth + this.ball.radius &&
               Math.abs(localY) <= halfHeight + this.ball.radius;
    }
    ...
    draw() {
        ...
        // Draw left paddle (rotated -45 degrees)
        this.ctx.save();
        this.ctx.translate(this.leftPaddle.x + this.leftPaddle.width/2, this.leftPaddle.y + this.leftPaddle.height/2);
        this.ctx.rotate(-Math.PI / 4); // -45 degrees
        this.ctx.fillRect(-this.leftPaddle.width/2, -this.leftPaddle.height/2, this.leftPaddle.width, this.leftPaddle.height);
        this.ctx.restore();
        
        // Draw right paddle (rotated 45 degrees)
        this.ctx.save();
        this.ctx.translate(this.rightPaddle.x + this.rightPaddle.width/2, this.rightPaddle.y + this.rightPaddle.height/2);
        this.ctx.rotate(Math.PI / 4); // 45 degrees
        this.ctx.fillRect(-this.rightPaddle.width/2, -this.rightPaddle.height/2, this.rightPaddle.width, this.rightPaddle.height);
        this.ctx.restore();
